...
const Step03 = () => {
	...
	useEffect(() => {
		const fetchStep03 = async () => {
			await axios
				.get(`${process.env.NEXT_PUBLIC_URL}/auth/getStep03?userId=${router.query.userId}&registrationNoticesId=${router.query.form}`)
				.then((res) => {
	...
	const handleDelete = async (itemId) => {
		try {
			const response = await axios.delete(
				`${process.env.NEXT_PUBLIC_URL}/auth/delStep03/${itemId}`
			);
		...
		<div className=' fade-in  h-full w-full'>
			<div className='  vh70 rounded-md m-1 px-8 text-center bg-indigo-900 bg-opacity-60 flex flex-col justify-between'>
				<div className=' w-full'>
								...
								{data.map((item) => (
									<tbody key={item.id} className='w-full'>
										<tr>
											...
											<td className={step03Td}>
												<img
													src={`${process.env.NEXT_PUBLIC_URL}/uploads/${item.fileGrade}`}
													alt={`${item.fileGrade}`}
													className=' w-28 h-24'
												/>
											</td>
					...
					<div>
						{data.length > maxLengh ? (
							<button
								onClick={() => {
									toast.error('شما مجاز به ثبت بیش از 4 رکورد نمی باشید');
								}}
				...
				<NextStep
					disableForm={data.length < minLengh}
					type={'submit'}
					onClick={fetchData}
				/>
			</div>
